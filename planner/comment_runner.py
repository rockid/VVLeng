"""Build the self-contained HTML 'comment runner' from a comment sheet CSV.

No server, no install — the operator double-clicks the .html to work the day's
sheet. It captures which comment variant was used per post and exports the
feedback CSV (``commented_log_{date}.csv``) that seeds the operator feedback
loop. Promoted from ``scratch/build_comment_ui.py`` (2026-07-05) so every run
...
def build_comment_runner(sheet_csv_path: str, out_path: str | None = None) -> str:
    """
    Render the HTML comment runner next to its comment sheet.

    Args:
        sheet_csv_path: Path to a ``comment_sheet_{date}.csv`` produced by
            ``write_comment_sheet``.
        out_path: Optional explicit output path. Defaults to the sheet path
            with ``comment_sheet_`` → ``comment_runner_`` and ``.csv`` → ``.html``.

    Returns:
        Path to the written HTML file.
    """
    if out_path is None:
        base = os.path.basename(sheet_csv_path)
        out_path = os.path.join(
            os.path.dirname(sheet_csv_path),
            base.replace("comment_sheet_", "comment_runner_").replace(".csv", ".html"),
        )

    # Title = the sheet's date if recoverable from the filename, else today.
    m = re.search(r"(\d{4}-\d{2}-\d{2})", os.path.basename(sheet_csv_path))
    title = m.group(1) if m else date.today().isoformat()

    with open(sheet_csv_path, encoding="utf-8") as f:
    ...
    html = _HTML_TEMPLATE.replace("__DATA__", data_json).replace("__TITLE__", title)

    with open(out_path, "w", encoding="utf-8") as f:
        f.write(html)

    logger.info("Comment runner written to %s (%d posts)", out_path, len(data))
    return out_path
