...
- [ ] `clients/alumni_saas.yaml` exists with keyword list populated
...
```bash
python run_pipeline.py --client alumni_saas --dry-run
```

**What to check:**
- Prints all pipeline steps without calling any API
- Creates `data/alumni_saas/` with all subdirectories
- No import errors, no missing key errors
- Confirm it reads from `clients/alumni_saas.yaml` (add a print statement in `config_loader.py` temporarily if unsure)
...
```bash
python run_pipeline.py --client alumni_saas --init-db-only
```

**What to check:**
- No errors
- `data/alumni_saas/engagement.db` (or wherever `DATABASE_URL` points) exists
...
2. Search for actor: `harvestapi/linkedin-post-search`
3. Click "Try for free" / "Run"
4. Enter this minimal input:
   ```json
   {
     "searchTerms": ["alumni network"],
     "maxResults": 5
   }
   ```
5. Run it. Wait for completion (usually 1–3 min).
6. Go to the dataset output. Look at the results.

**What to check:**
- Did it return any posts at all? If 0 results → actor may be broken or need different input format. Try `apimaestro/linkedin-posts-search-scraper-no-cookies` instead.
- Are the posts relevant to "alumni network"? (rough check, not scientific)
- Click on one result item and copy the full JSON. Save it as `tests/fixtures/sample_post_search.json`

### 3b. Map the field names

Open the saved JSON item. Find the actual field names for:

| What we need | Likely field name (check your actual output) |
|---|---|
| Post ID | `id`? `postId`? `urn`? |
| Post URL | `url`? `postUrl`? `link`? |
| Post text | `text`? `content`? `commentary`? |
| Author name | `authorName`? `author.name`? |
| Author headline | `authorHeadline`? `author.headline`? |
| Author profile URL | `authorUrl`? `author.profileUrl`? |
| Likes count | `likesCount`? `likes`? `reactions`? |
| Comments count | `commentsCount`? `comments`? |
| Posted date | `postedAt`? `date`? `publishedAt`? |

Paste the raw JSON item to Claude and say: *"Map these fields to the `HARVESTAPI_FIELD_MAP` in `normaliser.py`."* Claude will produce the corrected mapping dict.

### 3c. Update normaliser.py

Give Cline the corrected field mapping. Cline updates `normaliser.py`. This must happen before any pipeline code runs against real Apify output.

**If harvestapi returns 0 results:** Repeat steps 3a–3c with `apimaestro/linkedin-posts-search-scraper-no-cookies`. Update `config.yaml` actor ID accordingly.
...
```bash
python run_pipeline.py --client alumni_saas --stage collect --keywords "alumni network" --max-posts 5
```
...
config = load_config("alumni_saas")
...
- Posts written to `data/alumni_saas/raw/raw_{date}.json`
...
with open(f"data/alumni_saas/raw/raw_{today}.json") as f:
...
```bash
python run_pipeline.py --client alumni_saas \
...
- `data/alumni_saas/raw/` — raw JSON written
- `data/alumni_saas/output/` — `signal_{date}.csv` written
- `data/alumni_saas/plans/` — daily plan JSON written
...
2. Populate `influencer_watchlist` in client YAML with 10–15 real LinkedIn profile URLs of known relevant accounts
...
- Daily plan has <5 actions → check keyword engagement thresholds, they may be too high for low-volume niche
...
|---|---|---|
| Normaliser returns None fields | Actor field names don't match mapping | Re-do Step 3b, paste raw JSON to Claude |
